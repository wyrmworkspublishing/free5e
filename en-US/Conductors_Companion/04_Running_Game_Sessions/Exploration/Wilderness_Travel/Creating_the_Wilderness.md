...
To create an adventure framework for a wilderness journey, use the following steps.
Random tables can generate locations, encounter ideas, and landmark suggestions.

#### Determine the Weather (Optional)
Weather can add atmosphere or meaningfully shape travel and encounters.
Apply mechanical effects only when they constrain options, introduce risk, or reinforce the environment; otherwise, treat weather descriptively.

Choose the **dominant environment**: Arid (desert), Temperate (plains/forest), Wet (swamp/jungle), Cold (taiga/tundra), Mountain (alpine), Coastal (shore/sea).

##### Temperature

Set temperature by season and environment.
Apply effects only when exposure matters.

* **Frigid.** Each hour of exposure, creatures without cold-weather gear or cold resistance must succeed on a DC 10 Constitution saving throw or gain 1 level of exhaustion. Gear or cold resistance grants automatic success.  
* **Mild/Warm.** No mechanical effect.  
* **Scorching.** Each hour of exertion, creatures without shade or water must succeed on a Constitution saving throw (DC 10 + 1 per previous check that day) or gain 1 level of exhaustion. Fire resistance grants automatic success; heavy armor or heavy clothing imposes disadvantage.

**Optional fluctuation:** Once per day, roll 1d6; on a 1 shift one step colder, on a 6 shift one step warmer (within seasonal and environmental limits).

##### Weather States

Start at **Clear**, unless the Conductor decides otherwise.
Apply effects while the state persists.

| Step | Weather State | Effects |
| ----: | :---- | :---- |
| A | Clear | — |
| B | Cloudy | — |
| C | Fog/Mist | Beyond 60 ft lightly obscured; Perception checks relying on sight have disadvantage. |
| D | Light Precipitation | — |
| E | Steady Precipitation | Open flames extinguished; Perception checks relying on hearing have disadvantage. |
| F | Heavy Precipitation | Beyond 60 ft lightly obscured; Perception checks relying on sight or hearing have disadvantage. |
| G | Storm | As Heavy Precipitation; may trigger Extreme Weather. |

Precipitation type: Mild+ rain • Cold snow/sleet • Frigid snow/ice.

##### Weather Change

At intervals chosen by the Conductor (each watch, twice per day, or once per day), roll **1d10** and move along the table above.

| d10 | Result |
| ----: | :---- |
| 1 | Move two steps toward calmer conditions |
| 2–3 | Move one step toward calmer conditions |
| 4–6 | No change |
| 7–8 | Move one step toward harsher conditions |
| 9–10 | Move two steps toward harsher conditions; if ending at F or G, check Extreme Weather |

Movement follows the table and cannot go past A or G.

When weather **enters** a precipitation state (D–G), roll **1d6** for onset: 1 dawn, 2 morning, 3 midday, 4 afternoon, 5 evening, 6 night.

##### Wind

Wind usually follows the Weather State, but may be rolled separately if needed.

* **A–E:** Light/Moderate (no default effect)  
* **F:** Strong  
* **G:** Gale

For **Strong or Gale**, determine direction (1d8).
Within **45°** of the wind’s direction (e.g., N includes NE and NW), creatures move with it (+5 ft per 10 ft) or against it (−5 ft per 10 ft; minimum 5); crosswind movement is unaffected.
Strong wind disperses fog and smoke.
Gale also moves unsecured objects; flying creatures moving against the wind must advance at least 10 ft or descend 30 ft (landing if possible) or fall.
At long range, ranged attacks take −2; if airborne debris, spray, or sand is present, −5.

##### Extreme Weather

When a **Storm** escalates, weather may become an encounter or increase the danger of an existing one.
Use **Exploration Encounter Descriptions** (Chapter XX: Exploration) for hazards such as flooding, whiteouts, ice storms, sandstorms, cyclonic winds, or lightning activity.
Use extreme weather to change routes, limit options, or impose time pressure.

##### Magical Weather (Optional)

In magically active regions, weather may manifest supernatural effects.
When appropriate, apply one entry from the **Spell Effects** table (Chapter XX: Treasure) as an environmental condition for as long as the current Weather State persists.
Treat the effect as originating from the environment, not a creature.

#### Determine Potential Encounters
As the characters travel through the wilderness, choose or randomly select potential encounters.
These might be face-to-face encounters with denizens of the wilderness, but not all such encounters need to be hostile.
The characters could run into friendly travelers, fearful monsters, weak foes, or signs of a previous battle.
Likewise, they might spot the tracks of creatures recently passed by, or that are heading in the party's direction.

#### Place Notable Landmarks
Use notable landmarks to mark key points along the characters' journey.
